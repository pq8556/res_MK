...
class Encoder_MK(object):
    def __init__(self, x, y, adjust_scale=0, reg_pt_scale=1.0, drop_ge11=False, drop_ge21=False, drop_me0=False, drop_irpc=False, drop_MK=False, nbits=7, input2=False, nanStd=1):
        if x is not None and y is not None:
            ...
            # MK input2 for CNN
            self.input2_phi = self.x_copy[:,0:5].copy()
            self.input2_theta = self.x_copy[:,12+0:12+5].copy()
            self.input2_bend = self.x_copy[:,12*2+0:12*2+5].copy()

            
            # normalizing
            #self.input2_phi /= 5000
            #self.input2_theta /= 90
            
            self.input2_phi = np.nan_to_num(self.input2_phi)
            self.input2_theta = np.nan_to_num(self.input2_theta)
            self.input2_bend = np.nan_to_num(self.input2_bend)
    ...
    def get_x(self, drop_columns_of_zeroes=True,input2=False):
        ...
        if drop_columns_of_zeroes:
            ...
            if input2 == False:
                ...
                x_newer = np.hstack((x_new, self.input2_phi, self.input2_theta, self.input2_bend))
                return x_newer
...
def pt_range_select(ptchan,ptmin,ptmax):
    # here pt is actually 1/pt one over pt
    indpt = (((ptchan > ptmin) & (ptchan < ptmax)) | ((ptchan < -ptmin) & (ptchan > -ptmax)))
    return indpt