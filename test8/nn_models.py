...
# MK:
def MKmasked_binary_crossentropy(y_true, y_pred, mask_value=0.5):
    xx = K.binary_crossentropy(y_true, y_pred)
    mask = K.not_equal(y_true, mask_value)
    mask = K.cast(mask, K.floatx())
    xx *= mask
...
#================================================================
# ______________________________________________________________________________
def create_model_bn_charge(nvariables, lr=0.001, clipnorm=10., nodes1=64, nodes2=32, nodes3=16, discr_loss_weight=1.0,
                    l1_reg=0.0, l2_reg=0.0, use_bn=True, use_dropout=False):
    regularizer = regularizers.L1L2(l1=l1_reg, l2=l2_reg)
    inputs = Input(shape=(nvariables,), dtype='float32')
    ...
    charge_prediction = Dense(1, activation='sigmoid', name='charge_prediction')(x)
  
    # Create model
    #MK: adding charge prediction output
    model = Model(inputs=inputs, outputs=[regr, discr, charge_prediction])
  
    # Set loss and optimizers
    adam = optimizers.Adam(lr=lr, clipnorm=clipnorm)
    # MK: loss function specification for charge
    model.compile(optimizer=adam,
      loss={'regr': masked_huber_loss, 'discr': masked_binary_crossentropy, 'charge_prediction': MKmasked_binary_crossentropy},
      loss_weights={'regr': 1.0/discr_loss_weight, 'discr': 1.0, 'charge_prediction': 1.0},
      #metrics={'regr': ['acc', 'mse', 'mae'], 'discr': ['acc',]}
      )
    model.summary()
    return model
...
def create_model_bn_charge_cnn(nvariables, lr=0.001, clipnorm=10., nodes1=64, nodes2=32, nodes3=16, discr_loss_weight=1.0,
                               l1_reg=0.0, l2_reg=0.0, use_bn=True, use_dropout=False):
    regularizer = regularizers.L1L2(l1=l1_reg, l2=l2_reg)
    #inputs = Input(shape=(nvariables,), dtype='float32')
    # MK
    ...
    x = Dense(nodes1, kernel_initializer='glorot_uniform', kernel_regularizer=regularizer, use_bias=False)(dnn_input)
    if use_bn: x = BatchNormalization(epsilon=1e-4, momentum=0.9)(x)
    x = Activation('tanh')(x)
    if use_dropout: x = Dropout(0.2)(x)
    if nodes2:
        ...
        if nodes3:
            ...
            x = Dense(nodes3, kernel_initializer='glorot_uniform', kernel_regularizer=regularizer, use_bias=False)(concatenated)
            if use_bn: x = BatchNormalization(epsilon=1e-4, momentum=0.9)(x)
            x = Activation('tanh')(x)
            if use_dropout: x = Dropout(0.2)(x)
    ...
    # Create model
    #MK: adding charge prediction output
    model = Model(inputs=[dnn_input,cnn_input], outputs=[regr, discr, charge_prediction])
