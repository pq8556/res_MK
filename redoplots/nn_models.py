...
# ______________________________________________________________________________
# Save/Load models
#MK: add direcotry so as not to clutter by separating savedmodels related files
def save_my_model(model, name='model', dir_out='./savedmodels/'):
    # Store model to file
    #model.summary()
    model.save(dir_out+name + '.h5')
    model.save_weights(dir_out+name + '_weights.h5')
    # Store model to json
    with open(dir_out+name + '.json', 'w') as outfile:
        outfile.write(model.to_json())
    return

def load_my_model(name='model', weights_name='model_weights', dir_out='./savedmodels/'):
    with open(dir_out+name + '.json', 'r') as f:
        json_string = json.dumps(json.load(f))
        model = model_from_json(json_string)
    #model = load_model(name + '.h5')
    model.load_weights(dir_out+weights_name + '.h5')
    return model
