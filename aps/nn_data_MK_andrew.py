...
def muon_data(filename, adjust_scale=0, reg_pt_scale=1.0, correct_for_eta=False,nbits=7):
  try:
    logger.info('Loading muon data from {0} ...'.format(filename))
    loaded = np.load(filename)
    the_variables = det_bits(loaded,ind_det=24,nbits=nbits,verbose=True)
  ...
  encoder = Encoder_MK(the_variables, the_parameters, adjust_scale=adjust_scale, reg_pt_scale=reg_pt_scale, drop_MK=True, nbits=nbits)
  if correct_for_eta:
...
def muon_data_split(filename, adjust_scale=0, reg_pt_scale=1.0, test_size=0.5, correct_for_eta=False,nbits=7):
  x, y, w, x_mask = muon_data(filename, adjust_scale=adjust_scale, reg_pt_scale=reg_pt_scale, correct_for_eta=correct_for_eta,nbits=nbits)
